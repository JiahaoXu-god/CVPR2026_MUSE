# this code inputs the original slide path, and utilzes the simple algorithms to 
# remove the background of each slide. And returns the coords information of each
# patch.
...
def stitching(
	...
	"""
	get the downscale picture of the slide.
	"""
...
def segment(
	...
	"""
 	segment the slide and return mask.
  	"""
	### Start Seg Timer
...
def patching(
    ...
    **kwargs
	...
	"""
	generate the h5 file containing coords.
	"""
	### Start Patch Timer
	...
	try:
		...
		# 如果没有该属性，尝试从 openslide.objective-power 获取
		magnification = WSI_object.wsi.properties.get('openslide.objective-power', None)
		if magnification is None:
			# 如果仍然无法获取，设置默认放大倍数
			magnification = 40  # 根据实际情况设置默认值
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, only_mask_save_dir, stitch_save_dir,
				  ...
				  patch = False, auto_skip = True, process_list = None, uuid_name_file = None,
      			  is_uuid_inpath = True, file_ext = ''):

# get the relationship between slide and uuid name
	...
	if is_uuid_inpath:
		slides = [slide for slide in slides if os.path.isfile(os.path.join(source, id_names[str(slide)], slide+file_ext))]
	else:
		slides = [slide for slide in slides if os.path.isfile(os.path.join(source, slide+file_ext))]
	...
	for i in range(total):
		...
		# Inialize WSI
		if is_uuid_inpath:
			full_path = os.path.join(source, id_names[slide], slide+file_ext)
		else:
			full_path = os.path.join(source, slide+file_ext)
...
def parse_option():
	...
	parser.add_argument('--is_uuid_inpath', default=True, action='store_false', 
						help='whether the uuid is in the path of the slide name file')
	parser.add_argument('--file_ext', type=str, default='', 
						help='define the file extension of the slide name file')
...
if __name__ == '__main__':
	...
	seg_times, patch_times = seg_and_patch(**directories, **parameters,
										   ...
										   is_uuid_inpath = args.is_uuid_inpath, file_ext = args.file_ext)
