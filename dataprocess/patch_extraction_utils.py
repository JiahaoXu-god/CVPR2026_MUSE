...
# 获取项目根目录（假设你的 CONCH 文件夹和 my_project 同级）
...
def save_embeddings(model, fname, dataloader, enc_name, overwrite=False):
    ...
    for batch, coord in dataloader:
        with torch.no_grad():
            ...
            if enc_name == 'conch':
                embeddings.append(model.encode_image(batch, proj_contrast=False, normalize=False).cpu().numpy().squeeze())
            elif enc_name == 'musk':
                embeddings.append(model(batch)[0].detach().cpu().numpy().squeeze())
            elif enc_name == 'plip':
                batch = batch.squeeze(1)
                #a = model(pixel_values=batch)
                #print(a)
                embeddings.append(model(pixel_values=batch)[1].detach().cpu().numpy().squeeze())
            elif enc_name == 'gigapath':
                embeddings.append(model(batch).detach().cpu().numpy().squeeze())
                #print(embeddings[0].shape)
            elif(enc_name != 'clip_RN50' and enc_name != 'clip_ViTB32'):
                embeddings.append(model(batch).detach().cpu().numpy().squeeze())
            else:
...
def create_embeddings(embeddings_dir, enc_name, dataset, batch_size, save_patches=False,
    ...
    if enc_name == 'resnet50_trunc':
        ...
        model, preprocess = create_model_from_pretrained("conch_ViT-B-16", checkpoint_path="/data2/dongjiajun/code/xjh_few_shot_WSI_final/CONCH/checkpoints/conch/pytorch_model.bin")
        eval_t = eval_transforms_clip(pretrained=True)
    
    elif enc_name == 'musk':
        model, eval_t = pre_extracted_image_feature_musk()
        
    elif enc_name == 'plip':
        local_plip_path = '/home/dongjiajun/.cache/huggingface/hub/models--vinid--plip/snapshots/67ade53ddd32195868f422585f72698ef5d15094'
        model = CLIPModel.from_pretrained(local_plip_path, local_files_only=True).vision_model
        eval_t = CLIPProcessor.from_pretrained(local_plip_path, local_files_only=True)
    ...
    if(enc_name != 'clip_ViTB32' and enc_name != 'clip_RN50' and enc_name != 'conch'):
    ...
    if dataset == 'TCGA':
        ...
        for wsi_name in tqdm(os.listdir(patch_datasets)):
            dataset = PatchesDataset(os.path.join(patch_datasets, wsi_name), transform=eval_t, enc_name=enc_name)
...
class PatchesDataset(Dataset):
    def __init__(self, file_path, transform=None, enc_name=None):
...
def pre_extracted_image_feature_musk():
    import torchvision
    from timm.data.constants import IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD
    from timm.models import create_model
    from MUSK.musk import modeling
    from MUSK.musk import utils as mutils
    import os

    local_path = os.path.join("/data2/dongjiajun/code/xjh_few_shot_WSI_final/musk")
    ...
    mutils.load_model_and_may_interpolate(
        "hf_hub:xiangjx/musk", model, "model|module", "", local_path
    )
